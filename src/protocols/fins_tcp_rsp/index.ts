import {
	...
	createInputLengthSimple as numeric_input_len,
...
import { VecField, LoopField } from "../../field/vec"
...
const MultipleMemoryAreaReadItemChoice = new StructEnum(
	...
	[
		new AnonymousStructVariant(
			...
			[bytesRef('item', createCountVar('1 as usize'))]
		),
		new AnonymousStructVariant(
			...
			[bytesRef('item', createCountVar('2 as usize'))]
		),
		new AnonymousStructVariant(
			'0xC0 | 0xDC | 0xDD | 0xF0 | 0xF1 | 0xF2',
			'MultipleMemoryAreaReadItem4',
			[bytesRef('item', createCountVar('4 as usize'))]
		),
	],
...
const ConnectionDataReadDataItem = new Struct(
	...
	[
		...
		bytesRef('model_number', createCountVar('20 as usize'))
	]
)
...
const FileNameReadDiskDataItem = new Struct(
	...
	[
		bytesRef('volume_label', createCountVar('12 as usize')),
		numeric('date', 'be_u32'),
...
const CycleTimeReadChoice = new StructEnum(
	...
	new BasicEnumChoice(numeric_input_len('input_len'))
)
structs.push(CycleTimeReadChoice)

const CycleTimeReadItem = new Struct(
	'CycleTimeReadItem',
	[
		numeric_input_len('input_len'),
		new EnumField(CycleTimeReadChoice)
	]
)
structs.push(CycleTimeReadItem)
...
const AccessRightAcquireItem = new Struct(
	'AccessRightAcquireItem',
	[
		numeric_input_len('input_len'),
		new EnumField(AccessRightAcquireChoice)
	]
)
structs.push(AccessRightAcquireItem)

const MessageInfo = new Struct(
	'MessageInfo',
	[
		bytesRef('item', createCountVar('32 as usize'))
	]
)
...
const MessageReadOrClearOrFALSReadChoice = new StructEnum(
	...
	[
		new AnonymousStructVariant(0x14, 'MessageReadOrClearOrFALSRead20', [
			numeric('rsp_code', 'be_u16'),
			numeric('fals', 'be_u16'),
			bytesRef('error_message', createCountVar('16 as usize')),
		]),
...
const MessageReadOrClearOrFALSReadItem = new Struct(
	'MessageReadOrClearOrFALSReadItem',
	[
		numeric_input_len('input_len'),
		new EnumField(MessageReadOrClearOrFALSReadChoice)
	]
)
structs.push(MessageReadOrClearOrFALSReadItem)
...
const ControllerDataReadDataChoice = new StructEnum(
	...
	[
		new AnonymousStructVariant(0xa1, 'ControllerDataReadDataItem161', [
			numeric('rsp_code', 'be_u16'),
			bytesRef('controller_model', createCountVar('20 as usize')),
			bytesRef('controller_version', createCountVar('20 as usize')),
			bytesRef('for_system_use', createCountVar('40 as usize')),
			...
			bytesRef('cpu_bus_rsserved', createCountVar('32 as usize')),
...
const ControllerDataReadDataItem = new Struct(
	'ControllerDataReadDataItem',
	[
		numeric_input_len('input_len'),
		new EnumField(ControllerDataReadDataChoice)
	]
)
structs.push(ControllerDataReadDataItem)
...
const Order = new StructEnum(
	...
	[
		...
		new AnonymousStructVariant(0x0104, 'MultipleMemoryAreaRead', [
			numeric('rsp_code', 'be_u16'),
			//一种新的生成方式，传入一个StructA,一个变量名XXX，生成
			/*
			1.let mut input = input;
			2.let mut XXX = Vec::new();
			3.let mut A:StructA
			loop{
				if input.len()<=0{
					break;
			...
			*/
			new LoopField(MultipleMemoryAreaReadItem, 'data')
			//new StructField(MultipleMemoryAreaReadItem, 'data')
		]),
		...
		new AnonymousStructVariant(0x0220, 'DataLinkTableRead', [
			...
			new LoopField(DLTBLockDataItem, 'data'),
			//new VecField("block_record", createCountVarWithUnitSize("input.len()", 8, 'div'), DLTBLockDataItem)
		]),
		...
		new AnonymousStructVariant(0x0501, 'ControllerDataRead', [
			new StructField(ControllerDataReadDataItem)
		]),

		new AnonymousStructVariant(0x0502, 'ConnectionDataRead', [
			numeric('rsp_code', 'be_u16'),
			numeric('number_of_units', 'u8'),
			new LoopField(ConnectionDataReadDataItem, 'data'),
			//new VecField('data', createCountVarWithUnitSize('input.len()', 21, 'div'), ConnectionDataReadDataItem)
		]),
		...
		new AnonymousStructVariant(0x0602, 'NetworkStatusRead', [
			numeric("rsp_code", 'be_u16'),
			bytesRef("network_nodes_status", createCountVar('31 as usize')),
			...
			bytesRef("network_nodes_non_fatal_error_status", createCountVar('8 as usize')),
			bytesRef("network_nodes_cyclic_error_counters", createCountVar('62 as usize')),
		]),
		...
		new AnonymousStructVariant(0x0620, 'CycleTimeRead', [
			new StructField(CycleTimeReadItem)
		]),
		...
		new AnonymousStructVariant(0x0920, 'MessageReadClearFALSRead', [
			new StructField(MessageReadOrClearOrFALSReadItem)
		]),

		new AnonymousStructVariant(0x0c01, 'AccessRightAcquire', [
			new StructField(AccessRightAcquireItem)
		]),
		...
		new AnonymousStructVariant(0x2102, 'ErrorLogRead', [
			...
			new LoopField(ErrorLogReadDataItem, 'error_log_data')
		]),
		...
		new AnonymousStructVariant(0x2201, 'FileNameRead', [
			...
			new LoopField(FileNameReadFileDataItem, 'error_log_data')
		]),
		...
		new AnonymousStructVariant(0x220f, 'FileMemoryIndexRead', [
			...
			new LoopField(FileMemoryIndexReadDataItem, "data")
...
export const FinsTcpRspPacket = new Struct(
	`${packetName}`,
	[
